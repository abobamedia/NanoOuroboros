# Ouroboros v4.18.15 — Architecture & Reference
...
- **Model**: `openai/gpt-5.5-pro` via OpenRouter (primary), `openai::gpt-5.5-pro` via direct OpenAI (fallback), or `openai-compatible::gpt-5.5` via the configured OpenAI-compatible provider. If none of those keys are configured, the review tool returns an availability error.
